...
frappe.ui.form.on("Multiple Cheque Entry", {
    setup: function(frm) {
        ...
        frm.set_query("account_currency", () => ({
...
frappe.ui.form.on("Multiple Cheque Entry", {
    ...
    cheque_bank: function(frm) {
        ...
        frm.set_value("account_currency", "");
    },
    ...
    validate: function(frm) {
        ...
        if (!frm.doc.account_paid_to) {
            frappe.throw("Account Paid To is required");
        }
        if (!frm.doc.account_currency) {
            frappe.throw("Account Currency is required");
        }
    }
});
    ...
    table.forEach(row => {
        if (!row.payment_entry) {
            docs.push({
                ...
                paid_to: frm.doc.account_paid_to, // Use new field
                paid_to_account_currency: frm.doc.account_currency, // Use new field
